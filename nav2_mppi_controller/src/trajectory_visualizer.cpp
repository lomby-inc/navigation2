...
void TrajectoryVisualizer::on_configure(
  ...
  traj_plan_ = std::make_shared<nav_msgs::msg::Path>();
  ...
  trajectories_publisher_ =
      node->create_publisher<visualization_msgs::msg::MarkerArray>(
          "/trajectories", 1);
  trajectory_path_pub_ =
      node->create_publisher<nav_msgs::msg::Path>("mppi_trajectory_path", 1);
  ...
  auto getParam =
      parameters_handler->getParamGetter(name + ".TrajectoryVisualizer");
...
void TrajectoryVisualizer::add(const xt::xtensor<float, 2> &trajectory,
  ...
  auto add_marker = [&](auto i) {
    ...
    auto marker = utils::createMarker(marker_id_++, pose, scale, color,
...
void TrajectoryVisualizer::add(const models::Trajectories &trajectories,
  ...
  points_->markers.reserve(floor(shape[0] / trajectory_step_) *
                           floor(shape[1] * time_step_));
  ...
  for (size_t i = 0; i < shape[0]; i += trajectory_step_) {
    for (size_t j = 0; j < shape[1]; j += time_step_) {
      ...
      auto pose =
          utils::createPose(trajectories.x(i, j), trajectories.y(i, j), 0.03);
...
void TrajectoryVisualizer::visualize(const nav_msgs::msg::Path &plan) {
  ...
  if (trajectory_path_pub_->get_subscription_count() > 0) {
    trajectory_path_pub_->publish(*traj_plan_);
  }
